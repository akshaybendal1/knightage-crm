...
public class PipelineStageRepository : IPipelineStageRepository
{
    ...
    {
        const string sql = @"SELECT Id, Name, SortOrder, IsActive FROM PipelineStages WHERE IsActive = 1 ORDER BY SortOrder";
    ...
    {
        const string sql = @"SELECT Id, Name, SortOrder, IsActive FROM PipelineStages WHERE Id = @Id";
        using var connection = _context.CreateConnection();
    ...
    {
        const string sql = @"INSERT INTO PipelineStages (Id, Name, SortOrder, IsActive) VALUES (@Id, @Name, @SortOrder, @IsActive)";
        using var connection = _context.CreateConnection();
